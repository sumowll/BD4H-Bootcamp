...
- Can construct data for machine learning application.
- Can run basic classification and regression algorithms.
{% endobjective %}
...
## Overview
To apply machine learning algorithms provided in MLlib, we need to transform our data into `RDD[LabeledPoint]`. This feature construction is similar to what we did in [Hadoop Pig]({{ site.baseurl }}/hadoop-pig/), but will be more concise.
Recall that our data file is in the following form.
...
## Load data
The file **case.csv** consists of only positive examples, and the file **control.csv** consists of only negative examples. The function `union` is used to combine the two files. Since the data will be used more than once, we use `cache()` to prevent reading in the file multiple times.
...
## Unique features
Now we need to extract the set of distinct `event-id`'s, and assign each of them a consecutive integer value starting from 0. The first step is to extract all `event-id`'s, and then use `distinct()` to eliminate duplicate elements. Then, we append each element in the set a consecutive integer index by calling `zipWithIndex()`. Finally, in order to be used in the next step, it is convenient to transform this set (consists of pairs) into a Scala Map and collect to driver.
...
## Name to id
Next, we transform each line `(patient-id, event-id, timestamp, value)` into a key-value pair `((patient-id, feature-id), value)`, where `feature-id` is an integer mapped from the original `event-id`. Note that the key in this key-value pair is itself a pair.
Then, we apply `reduceByKey` to sum up `value`'s for each `(user-id, feature-id)` key. 
...
scala> val features = pos.map{ x =>
                        ...
                        // featureMap: event-id (String) => feature-id (Long)
...
## Create `LabeledPoint`
In the last step, we transformed each line into `((patient-id, feature-id), value)`, where `value` is actually an aggregated value. The next step is to group records with the same `patient-id`. To do this, we first transform `((patient-id, feature-id), value)` into `(patient-id, (feature-id, value))` and apply `groupByKey`. The `groupByKey` function collects all `(feature-id, value)` pairs associated with each `patient-id` into a collection of type `Iterable[(Int, Double)]`.
...
```
## Train classifier
Let's train a linear SVM model using Stochastic Gradient Descent (SGD) on the training set
